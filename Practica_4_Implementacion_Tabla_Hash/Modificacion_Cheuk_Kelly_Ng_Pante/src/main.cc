...
int main() {
  ...
  do {
    ...
    std::cout << kRedBold << "  [5]." << kReset << kBold << " Modificacion" << std::endl;
    ...
    switch (option) {
      ...
      case 4:
        ...
        exit(EXIT_SUCCESS);
      case 5: 
        // Generar 1000 elementos
        keyType aux_element;
        for (int i = 0; i < 1005; ++i) {
          aux_element = rand() % 10000;
          register_.push_back(aux_element);
        }

        for (int i = 0; i < 1000; ++i) {
          hashTable->insert(register_[i]);
        }
        std::cout << "Tamaño lista: " << hashTable->getCount() << std::endl;
        break;
      default:
